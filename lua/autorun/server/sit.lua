...
local function FindPose( this, me )
    ...
    ang = ang + 90 + 180
    ang = ang % 360
    table.sort( lookup, function( aa, bb ) return d( ang, aa[1] ) < d( ang, bb[1] ) end )
        ...
        if ( SitOnEntsMode:GetInt() >= 1 ) then
            if ( SitOnEntsMode:GetInt() == 1 ) then
                if ( not EyeTrace.HitWorld ) then
                    ...
                    if ( type( owner ) == "Player" and owner ~= nil and owner:IsValid() and owner:IsPlayer() ) then return end
                end
            end
            ...
            if ( SitOnEntsMode:GetInt() == 2 ) then
                if ( not EyeTrace.HitWorld ) then
                    local owner = EyeTrace.Entity:CPPIGetOwner()
                    if ( type( owner ) == "Player" and owner ~= nil and owner:IsValid() and owner:IsPlayer() and owner ~= ply ) then return end
                end
            end
        end
    end
...
local PickupAllowed = { "GravGunPickupAllowed", "PhysgunPickup" }
...
function CheckSeat( ply, ent, tbl )
    ...
    if ParentSeat:GetBool() then
        if vehicle.parent == ent then return false end
    end

    for _, v in next, ent:GetChildren() do
        if IsValid( v ) and not tbl[v] then
            tbl[v] = true
            if v ~= ent and CheckSeat( ply, v, tbl ) == false then return false end
        end
    end
...
local function CheckSeat2( ply, ent )
    ...
    if cache[ply:SteamID()] and cache[ply:SteamID()][ent:EntIndex()] and ( CurTime() - cache[ply:SteamID()][ent:EntIndex()][1] ) < 5 then return cache[ply:SteamID()][ent:EntIndex()][2] end
...
hook.Add( "AllowPlayerPickup", "Nopickupwithalt", function( ply )
    if ply:KeyDown( IN_WALK ) then return false end
end )
...
hook.Add( "InitPostEntity", "SAW_CompatFix", function()
    if hook.GetTable()["CanExitVehicle"]["PAS_ExitVehicle"] and PM_SendPassengers then
        local function IsSCarSeat( seat )
            if IsValid( seat ) and seat.IsScarSeat and seat.IsScarSeat == true then return true end
